...
class Classifier(Network):
    ...
    def backward(self, dout):
        ...
        # unpacking the variables from the cache
        X, y = self.cache

        # calculating the gradient of the loss function w.r.t. the output of the sigmoid function
        dL_dy = dout

        # calculating the gradient of the output of the sigmoid function w.r.t. the input of the sigmoid function
        dy_dx = y * (1 - y)

        # calculating the gradient of the loss function w.r.t. the input of the sigmoid function
        dL_dx = dL_dy * dy_dx

        # calculating the gradient of the loss function w.r.t. the weight matrix
        dW = np.dot(X.T, dL_dx)
